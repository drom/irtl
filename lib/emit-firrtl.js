...
const emitModule = node => {
  ...
  const defs = Object.keys(defo)
    .map(e => Object.assign({__ID__: e}, defo[e]));

  return ['module ' + node.__ID__ + ' :'].concat(indent([].concat(
    '; inputs',
    defs.filter(e => e.dir === 'input').map(e => e.dir + '  ' + emitType(e)),
    '; outputs',
    defs.filter(e => e.dir === 'output').map(e => e.dir + ' ' + emitType(e)),
    '; wires',
    defs
    ...
    '; body',
    node.items.map(item => item.__ID__ + ' <= ' + emitExpression(item, true))
  )));
};
